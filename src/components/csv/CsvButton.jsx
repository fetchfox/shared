...
export const CsvButton = ({ jobId, onStop, items, ...rest }) => {
  ...
  const controlsNode = (
    <div style={{ display: 'flex', gap: 8 }}>
      ...
      {!stopping && results && !results.done && <Button trans black tooltip="Stop" onClick={stop}>
  ...
  if (rest.children) {
    return (
      <div onClick={() => downloadCSV(items_)}>
        {rest.children}
      </div>
    );
  }
  ...
  return (
    <div
      ...
      {results && jobId && items_.length == 0 && !results?.done && (
        <Loading>Waiting for results...</Loading>
      )}
      {items_.length > 0 && <Button
