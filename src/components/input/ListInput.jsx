...
export const ListInput = (props) => {
  ...
  const nodes = props.value.map((v) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        <div style={{ width: '100%' }}>
          <Input style={{ width: '100%' }} key={index} value={props.value[index]} onChange={(e) => update(index, e)} />
        </div>
        <div
          style={{ display: 'flex', alignItems: 'center', color: '#888', cursor: 'pointer' }}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, width: '100%' }}>{nodes}</div>
      <div style={{ display: 'flex', margin: '6px 0 2px 0', color: '#888', cursor: 'pointer' }} onClick={add}>
        <div style={{ marginRight: 5, display: 'flex', alignItems: 'center' }}>
          <FaCirclePlus fontSize={18} />
        </div>
