...
const UrlsInput = ({ currentUrl, value, onChange, disabled }) => {
  ...
  if (!editing) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', color: '#555', paddingTop: 10, gap: 5 }}>
        <div
          style={{
            marginLeft: 10,
            fontWeight: 'bold',
        ...
        <MdEdit size={14} style={{ cursor: 'pointer' }} onClick={() => setEditing(true)} />
      </div>
    );
  } else {
  }
  ...
  return (
    <Input
      tabIndex="1"
      ref={inputRef}
      style={{ width: 'calc(100% - 8px)', boxShadow: 'unset', border: 0, background: 'white', margin: 4 }}
      value={value}
...
export const WorkflowPrompt = ({
  ...
  const handleKeyDown = (e) => {
    ...
    if ((e.ctrlKey && e.key == 'Enter') || (isMac && e.metaKey && e.key == 'Enter')) {
      handlePreview(e);
    }
  };
  ...
  if (overrideControls) {
    ...
    controlsNode = (
      <div style={{ display: 'flex', justifyContent: 'center', gap: 10, margin: '0 auto' }}>
        <Button
          large
          outline
          style={{ width: '50%' }}
  ...
  const formNode = (
    <form
      style={{ display: 'flex', flexDirection: 'column', gap: 5, background: '#ddd', borderRadius }}
    ...
    >
      <UrlsInput
        ...
        disabled={disabled}
      />
      <div style={{ position: 'relative', width: '100%', marginTopx: 8, opacity: loading.preview ? 0.5 : 1 }}>
        <div style={{ position: 'absolute', right: 2, bottom: 5, zIndex: 10 }}>
          {!disabled && (
            <Button
              tabIndex="3"
              type="submit"
              style={{ height: 30, width: 30, borderRadius: 15, padding: 0, boxShadow: 'unset' }}
  ...
  const codeNode = (
    <div style={{ background: '#ddd', borderRadius, padding: 10 }}>
      <div>
        <b>JSON Scraper Definition</b>
      </div>
      <div style={{ lineHeight: '20px' }}>
        Create a scraper using a JSON definition. For more information, see{' '}
        <a href="https://github.com/fetchfox/fetchfox">https://github.com/fetchfox/fetchfox</a>
      </div>
      <textarea
        style={{ width: '100%', height: 200, border: '1px solid #ccc', borderRadius: 5, marginTop: 15 }}
        value={code}
      ...
      {!workflow && (
        <Button loading={loading.code} onClick={handleCode} style={{ width: '100%', marginTop: 15 }}>
          Continue
        </Button>
      )}
    </div>
    ...
    <div style={{ width: '100%' }}>
      ...
      */}
      ...
      <div style={{ marginTop: 20 }}>
        ...
        {workflow && <p style={{ textAlign: 'center' }}>Review and edit the scrape plan below</p>}
        {workflow && <Workflow editable workflow={workflow} onChange={setWorkflow} onEditing={setEditing} />}
