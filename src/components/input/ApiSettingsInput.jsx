...
export const ApiSettingsInput = ({ settings, onSettingsChange }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', background: '#f6f6f6', borderRadius: 8, padding: 20 }}>
      <div style={{}}>
        <Choice
        ...
        >
          <div>
            Use FetchFox's servers as your AI provider. FetchFox will select which AI model to use and manage token
            usage and cost.
          </div>
        </Choice>
      </div>
      <div>
        ...
        >
          <div>
            Use a specific AI provider and model that you pick. You will incur costs from the AI provider, and you will
            receive bonus free extractions on FetchFox.
          </div>
          <ByokInput value={settings?.byok || {}} onChange={handleByok} />
        </Choice>
      </div>
    </div>
  );
